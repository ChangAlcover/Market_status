...
export function retrieve_efect_price (pair_name,operation,amount,limit_price, base_wss,depth=25) {
	...
	return new Promise (function(res, rej) {
		...
		.then(response => {
			const data = response;
			res(efect_price_logic(operation,amount,limit_price,depth,data));
		})
		.catch (err => {
			console.log("Error in function retrieve_bid");
		});
...
export function efect_price_logic (operation,amount,limit_price,depth,data) {
	
	var amount_limit=0;
	var amount_limit_value=0;
	var acumulate=0;
	var acumulate_value=0;
	/** Data have all the information for bid and ask**/
	if (operation==='sell')	{
		data=data[0];
	}
	if (operation==='buy')	{
